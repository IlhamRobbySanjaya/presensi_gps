...
class User extends CI_Controller
{
    ...
    private function _validasi($mode)
    {
        ...
        if ($mode == 'add') {
            ...
            $uniq_nip = $db['nip'] == $nip ? '' : '|is_unique[user.nip]';
            $uniq_email = $db['email'] == $email ? '' : '|is_unique[user.email]';

            $this->form_validation->set_rules('nip', 'nip', 'required|trim|alpha_numeric' . $uniq_nip);
    ...
    public function toggle($getId)
    {
        ...
        $pesan = $toggle ? 'user dinonaktifkan.' : 'user aktif.';
