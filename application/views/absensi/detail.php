                    ...
                    <div class="col-xs-12 col-sm-6">
                        <table class="table border-0">
                            <tr>
                                ...
                                <th class="border-0 py-0"><?= $pegawai->nama ?></th>
                            </tr>
                    ...
                    <div class="col-xs-12 col-sm-6 ml-auto text-right mb-2">
                        <div class="dropdown d-inline">
                            ...
                            <div class="dropdown-menu" aria-labelledby="droprop-action">
                                <a href="<?= base_url('presensi_online/export_pdf/' . $this->uri->segment(3) . "?bulan=$bulan&tahun=$tahun") ?>" class="dropdown-item" target="_blank"><i class="fa fa-file-pdf-o"></i> PDF</a>
                                <a href="<?= base_url('presensi_online/export_excel/' . $this->uri->segment(3) . "?bulan=$bulan&tahun=$tahun") ?>" class="dropdown-item" target="_blank"><i class="fa fa-file-excel-o"></i> Excel</a>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <?php if ($absen) : ?>
                            <?php foreach ($hari as $i => $h) : ?>
                                ...
                                <tr <?= (in_array($h['hari'], ['Sabtu', 'Minggu'])) ? 'class="bg-dark text-white"' : '' ?> <?= ($absen_harian == '') ? 'class="bg-danger text-white"' : '' ?>>
                                    <td><?= ($i + 1) ?></td>
                                    <td><?= $h['hari'] . ', ' . $h['tgl'] ?></td>
                                    <td><?= is_weekend($h['tgl']) ? 'Libur Akhir Pekan' : check_jam(@$absen_harian['masuk'], 'masuk') ?></td>
                                    <td><?= is_weekend($h['tgl']) ? 'Libur Akhir Pekan' : check_jam(@$absen_harian['istirahat_keluar'], 'istirahat_keluar') ?></td>
                                    <td><?= is_weekend($h['tgl']) ? 'Libur Akhir Pekan' : check_jam(@$absen_harian['istirahat_masuk'], 'istirahat_masuk') ?></td>
                                    <td><?= is_weekend($h['tgl']) ? 'Libur Akhir Pekan' : check_jam(@$absen_harian['pulang'], 'pulang') ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else : ?>
                            <tr>
